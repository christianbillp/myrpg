...
import {
  initSavesPersistence, savesDir, saveFilePath, carryForwardQuests,
  saveWorldState, loadWorldState, deleteWorldSave,
  readSave, readSaveIfExists, writeSave, ensureSaveExists,
  readAdventureSave, writeAdventureSave, deleteAdventureSave,
  type CharSave,
  type WorldSave,
} from "./persistence/saves.js";
...
initSavesPersistence({ dataDir: DATA_DIR, getDefs: () => defs });
...
server.post<{ Params: { characterId: string }; Body: { devFlags?: import("../../shared/types.js").DevFlags } }>(
  ...
  async (req, reply) => {
    ...
    const found = findSessionByCharacter(characterId);
    // Completion is engine-authoritative (combat victory or completionFlag) —
    // do not advance a live chapter that isn't finished. When no session is
    // live (server restart), the save's own bookkeeping is the best record.
    if (found && !found.session.engine.getState().encounterComplete) {
      return reply.code(409).send({ error: 'Chapter is not complete yet.' });
    }
...
async function startAdventureChapter(
  ...
  deleteSessionsForCharacter(engine.getState().player.defId);
...
server.get("/world", async (_req, reply) => {
  ...
  if (!engine) {
    ...
    deleteSessionsForCharacter(state.player.defId);

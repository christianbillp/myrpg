...
- \`kind\`: one of:
  ...
  - \`combat\` — flips monsters to enemy disposition + starts combat. Required: \`defIds\` (list of ids to flip). Each entry may be either a **bare def id** (e.g. \`"bandit"\`, \`"goblin_minion"\` — flips EVERY NPC in the encounter with that def, the usual "the bandits attack" shape) or a **runtime instance id** for a specific duplicate (e.g. \`"commoner_3"\` flips only the third commoner). Instance ids use the spawn-time naming: singletons keep the bare def, duplicates get \`\${defId}_\${ordinal}\` walked ally → enemy → neutral. DO NOT use slot refs (\`neutral_1\`, \`enemy_0\`) or combat labels (\`enemy_A\`) — those forms still aren't recognised.

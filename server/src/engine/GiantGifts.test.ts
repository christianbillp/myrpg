...
import { applyGiantGiftOnHit, giantGiftPoolMax, GIANT_GIFT_ID } from './GiantGifts.js';
...
function ctx(lineage: string, uses = 2, targetSize = 'large') {
  const r = buildTestContext({
    ...
    playerDef: { speciesId: 'goliath', speciesLineage: lineage, proficiencyBonus: 2 } as Partial<PlayerDef>,
  });

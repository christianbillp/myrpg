...
function fireAction(ctx: GameContext, action: TriggerAction): void {
  switch (action.type) {
    ...
    case 'trigger_combat': {
      ...
      // have at least one living enemy. Any entity_move events generated by an
      // NPC's first turn (if they outroll the player on Initiative) are lost
      // here — see the note in TriggerSystem header comment.
...
// ── Faction & rumor helpers (also called from AIDMTools) ─────────────────────

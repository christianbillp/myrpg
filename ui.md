...
`client/src/scenes/CharacterCreatorScene.ts` — a multi-step character-creation flow (US-122) reached from the **+ CREATE CHARACTER** button beneath the character carousel on the Encounter Setup screen. A full-screen stepped DOM panel with a step rail: **Concept → Origin → Abilities → Skills → Spells → Review** (the Spells step is skipped for non-casters). The **Origin** step also carries the SRD languages picker (US-123): every character knows Common and chooses two more from the Standard Languages table (class grants like the Rogue's Thieves' Cant are added automatically); known languages appear on the Character Sheet's Story tab. The **Concept** step is the AI-assist entry: the player types a free-text concept and *ASK THE AI* calls `POST /generate/character`, which honours the active setting's lore and returns a setting-consistent suggestion (species/background/class, name, tagline, backstory, and an ability priority); the suggestion pre-fills every later step and the player can edit anything. **Abilities** offers Standard Array, Point Buy (with a live remaining-points counter against the 27-point budget), or Roll (4d6-drop-lowest, rerollable), assigned via per-ability dropdowns or +/− steppers. Standard Array / Roll values are **single-use** — each row keeps a one-each permutation of the pool, and picking a value another ability holds swaps them. Every row shows the **final** score with the **origin bonus** broken out (e.g. *STR 15 +2 (Soldier) → 17 (+3)*) and an origin-bonus banner names the background and which abilities its +2/+1 raise, so the bonus baked into the final scores is always visible. **Skills** and the caster **Spells** step enforce their SRD counts. **Review** edits name/tagline/backstory + the A/B starting-gear choice and shows a stat summary. **✓ CREATE** posts the choices to `POST /characters`; on success the roster is re-fetched into the registry and the scene returns to Encounter Setup, where the new character appears in the carousel. CANCEL / ‹ BACK / NEXT › navigate the steps.

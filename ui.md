...
| **Companion Chip** | Single-line indicator visible whenever the player has a bound companion (NPC with `companion` set — promoted via the `set_npc_companion` trigger action or AIGM tool). Shows the companion's display name plus their current binding state — **FOLLOW** (auto-walks behind the player at ≥ 2-tile distance, picks up combat as an ally), **WAIT HERE** (idle on current tile), or a per-turn **ATTACK** override (forces the next combat target). Buttons on the chip cycle the binding and clear overrides. During combat the chip shows the combat label and re-uses the same `override` slot the ally AI reads in `NpcTurnRunners.ts`; out of combat the chip drives `FollowPlayerTask` / `WaitHereTask` from the NPC sim layer (US-094). When no companion is bound the chip is hidden entirely. |
...
| **Layer visibility toolbar** *(BOTTOM bar, above the paint buttons — aligned under the map column)* | Three small HTML chips: ZONES / TRIGGERS / MONSTERS. Click each chip to toggle that layer's visibility on the embedded preview — useful when zones + triggers + placement markers overcrowd the same view. Active chips render with the brighter "active" background; inactive chips dim. The state is per-`ZonePainter` instance and persists across paint mode changes; newly-painted cells and re-rendered placements honour the current visibility flags. |

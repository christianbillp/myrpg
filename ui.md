...
| **CAST**            | Varies       | Caster class (`spellcastingAbility` set), at least one castable spell available | Open the inline spell picker. Each entry shows the spell name, level tag (`cantrip` / `L1`), and a one-line mechanical summary (damage dice, save ability + DC, attack type, range/area). Selecting a spell sends a `castSpell` action with `targetIds = [selectedTargetId]` for attack-roll spells, no target for self/AOE spells. Cantrips spend no slot; levelled spells consume one slot of `spell.level − 1`. Shield is a reaction and is not exposed in this picker — it auto-fires when an incoming attack would land by ≤5 over AC. |
| **↩ CANCEL**        | —            | Throw or Spell picker is open                                                | Close the inline picker without spending any resource                   |

...
`client/src/scenes/CharacterCreatorScene.ts` — a multi-step character-creation flow (US-122) reached from the **+ CREATE CHARACTER** button beneath the character carousel on the Encounter Setup screen. A full-screen stepped DOM panel with a step rail: **Concept → Origin → Abilities → Skills → Spells → Review** (the Spells step is skipped for non-casters). The **Concept** step is the AI-assist entry: the player types a free-text concept and *ASK THE AI* calls `POST /generate/character`, which honours the active setting's lore and returns a setting-consistent suggestion (species/background/class, name, tagline, backstory, and an ability priority); the suggestion pre-fills every later step and the player can edit anything. **Abilities** offers Standard Array, Point Buy (with a live remaining-points counter against the 27-point budget), or Roll (4d6-drop-lowest, rerollable), assigned via per-ability dropdowns or +/− steppers. **Skills** and the caster **Spells** step enforce their SRD counts. **Review** edits name/tagline/backstory + the A/B starting-gear choice and shows a stat summary. **✓ CREATE** posts the choices to `POST /characters`; on success the roster is re-fetched into the registry and the scene returns to Encounter Setup, where the new character appears in the carousel. CANCEL / ‹ BACK / NEXT › navigate the steps.

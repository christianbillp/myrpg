...
| **CAVE terrain** | A natural cavern: 4 (or 6 with 5 ROOMS) organic chambers of varied rock floor (cave dust / gravel / rock) carved from void, linked by corridors and enclosed with correctly-rotated walls. Drops a water pool and a bottomless chasm (blocks sight) as hazards. Built on the shared `MapCanvas` + op toolbox (`composeCave`). |
| **TOWN terrain** | A paved settlement: varied stone paving (cobbles / bricks / slabs) with a central slab plaza, ringed by 2–6 wood-floored walled buildings whose doors connect to the plaza by paths. Emits a `plaza` zone + a `building <n>` zone per structure. Built on the shared `MapCanvas` + op toolbox (`composeUrban`). |

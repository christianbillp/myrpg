...
| **Choices Required** | Renders per `LevelUpChoicePrompt` from the preview. Wizard L2 surfaces a **Scholar Expertise** chip group (Arcana / History / Investigation / Medicine / Nature / Religion) and a **Spellbook spells** chip list with a `picked / count` counter — the chip list is omitted when the player already knows every wizard spell of a level they can cast. Fighter / Rogue L2 require no choices and the section reads "This level requires no player choices." |

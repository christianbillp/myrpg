...
| **CAST**            | Free         | Character is a caster (`PlayerDef.spellcastingAbility` set) | Shortcut button that opens the Character Sheet Overlay directly on the **Spells** tab so the player can pick a spell to cast without having to open CHARACTER and switch tabs first. Same teal (`#1a3a4a`) as TALK; sits immediately above TALK in the action stack. Action-economy and slot checks are still enforced by the Spells tab itself, so the button itself is always enabled for casters. Available during both `exploring` and `player_turn`. Hidden for non-casters. |

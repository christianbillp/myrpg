...
| **CAVE terrain** | A natural cavern built **hub-and-spoke** (distinct from the dungeon's grid): one large central chamber with 5 (or 7 with 5 ROOMS) small side chambers in the border ring, each tunnelled back to the centre. Varied rock floor (cave dust / gravel / rock) carved from void and enclosed with correctly-rotated walls; a pool + sight-blocking chasm sit in opposite corners of the central chamber. Emits a `central cavern` zone + a `chamber <n>` zone per side chamber. Built on the shared `MapCanvas` + op toolbox (`composeCave`). |

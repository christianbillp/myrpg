...
- **US-020** As a player, I want to be presented with an encounter setup menu before the game starts so that I can choose my encounter type(s), map type, and character. The Encounter Setup Scene has three columns: Encounter Type (multi-select — Simple Combat, Social Interaction, Exploration, and AI Dialogue can be combined in any combination; clicking a selected card deselects it), Map Type ("Open Map", "Rooms", or "Saved Map"), and Character (Aldric or Miriel). A BEGIN ENCOUNTER button activates once at least one encounter type, a map type, and a character are selected. All selected encounter types are active simultaneously on the same map.
...
- **US-023** As a player, I want to select a hand-crafted saved map so that I can play on a specific layout. Four maps are available: Arena, Dungeon, Ruins, and Catacombs. In the Encounter Setup screen the Map Type column shows a third card "Saved Map"; clicking it opens a picker overlay showing the four maps as cards. Clicking a map card confirms the selection immediately, closes the overlay, and displays the chosen map name on the card. The encounter cannot begin until a map is selected.
...
- **US-008** As a player, I want my game state saved and loaded automatically so that I can continue where I left off. Player state (HP, XP, GP, inventory, Second Wind charges) is persisted to `server/data/character.json` via the Fastify REST API (`POST /save` / `GET /save`) and cached in localStorage for fast synchronous reads. When no save exists the server derives default values from the first character file in `server/data/characters/`. The Encounter Setup screen shows a "Saved: …" banner with the saved character's stats and pre-selects that character. A NEW ENCOUNTER button in the HUD returns to the setup screen at any time, triggering the auto-save.
- **US-026** As a developer, I want all game content (monsters, NPCs, items, maps, characters) served from the Fastify REST API so that content can be updated centrally without redeploying the client. Static JSON files live under `server/data/` in per-type subdirectories (`monsters/`, `npcs/`, `items/`, `maps/`, `characters/`). The server exposes `GET /monsters`, `GET /npcs`, `GET /items`, `GET /maps`, and `GET /characters`; each endpoint reads all `.json` files in its directory and returns them as an array. The client preloads all five collections in `BootScene` before the first scene starts and stores them in the Phaser registry.
...
- **US-027** As a player, I can open a conversation with an AI dungeon master, so I can interact with the world through free form text explainations.
- **US-029** As a player, I can select from a list of premade encounters.....

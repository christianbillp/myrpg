...
- **US-021** As a player, I want to select a Social Interaction encounter so that I can engage with an NPC rather than fight. When the player is adjacent to any NPC and presses COMMUNICATE, a freeform AI chat overlay opens; replies are generated by Claude (claude-haiku-4-5) using the NPC's persona. If the AI is unavailable on the first message, the overlay falls back to the riddle system: a question with three answer options where a correct answer awards +10 GP. Each NPC maintains its own conversation history for the duration of the encounter. The `ai_dialogue` encounter type has been merged into `social_interaction`; all NPC interactions now use the same implementation.

...
- **US-021** As a player, I want to select a Social Interaction encounter so that I can engage with an NPC rather than fight. A Commoner (SRD 5.2.1 stat block) spawns on the map. When the player moves adjacent to the Commoner, a COMMUNICATE action button appears in the HUD. Clicking it presents a riddle with three answer options. A correct answer congratulates the player and awards +10 GP; a wrong answer shows a failure message. The TALK button disappears once the riddle has been answered.
...
- **US-023** As a player, I want to play on hand-crafted saved maps so that I can experience specific layouts. Four maps are available: Arena, Dungeon, Ruins, and Catacombs, stored as JSON in `server/data/maps/` and served via `GET /maps`. Each map is pre-processed server-side into a `passable` boolean grid. Map selection is now implicit in the premade encounter choice (each `PremadeEncounterDef` carries a `mapId`); the standalone map picker overlay is no longer used.
...
- **US-027** As a developer, I want NPCs to be a distinct data type from monsters so that non-combat characters can carry dialogue personas without polluting SRD stat blocks. NPCs live in `server/data/npcs/` as `NPCDef` objects with `id`, `name`, `monsterClass` (reference to a `MonsterDef` id for stat resolution), `color`, and an optional `persona` string. At spawn time the client merges the NPC's identity over the referenced monster's stat block. The `POST /npc/chat` endpoint looks up the NPC by id, resolves its persona, and forwards the conversation to the Claude API.
- **US-004** As a player, I want to select an AI Dialogue encounter so that I can have a freeform conversation with an AI-powered NPC. A Wandering Sage spawns on the map; when the player is adjacent, a COMMUNICATE button appears in the HUD. Clicking it opens a chat overlay: the player types freely, messages are sent to the server, and replies are generated by Claude (claude-haiku-4-5). Player movement and map zoom are both disabled while the overlay is open. The conversation history persists within the encounter — closing and re-opening the overlay resumes the same thread. The history area scrolls when text overflows, with mouse wheel or Up/Down arrow keys, and auto-scrolls to the bottom whenever a new message arrives. A proportional scrollbar thumb reflects the current scroll position.
...
- **US-029** As a player, I can select from a list of premade encounters so that I can replay encounters I enjoy. The Encounter Setup Scene is refactored into two columns: Character (left — Aldric or Miriel, same stat cards as before) and Premade Encounter (right — a 2×3 grid of encounter cards). Each encounter card shows the map name, title, encounter-type chips (colour-coded: Combat red, Exploration green, Social blue, AI Dialogue purple), and a flavour description. Premade encounters are JSON files in `server/data/premade-encounters/` served via `GET /premade-encounters`; each entry references a saved map by `mapId` and lists one or more `encounterTypes`. Six encounters ship by default: Arena Brawl, Dungeon Crawl, Ruins Mystery, Catacomb Hunt, The Sage's Counsel, Full Assault. The BEGIN ENCOUNTER button activates once a character and an encounter are both selected.
...
- **US-027** As a player, I can open a conversation with an AI dungeon master, so I can interact with the world through free form text explanations. The AI dungeon master (AIDM) is able to control all aspects of the encounter, such as xp, gold, hp, movement, actions etc. of enemies, npcs and players.

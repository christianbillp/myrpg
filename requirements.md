...
- **US-020** As a player, I want to be presented with an encounter setup menu before the game starts so that I can choose my encounter type, map type, and character. The Encounter Setup Scene has three columns: Encounter Type (currently "Simple Combat"), Map Type ("Open Map" with randomly scattered walls, or "Rooms" with connected rectangular rooms joined by single-tile corridors), and Character (Aldric Vane or Miriel Duskwhisper with full stat cards). A BEGIN ENCOUNTER button activates only once all three selections have been made.

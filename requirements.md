...
- **US-020** As a player, I want to be presented with an encounter setup menu before the game starts so that I can choose my encounter type and character. The Encounter Setup Scene shows two columns: the left column lists available encounter types (currently "Simple Combat" — a randomly generated map with a Goblin Minion and a Bandit), and the right column shows character cards for Aldric Vane and Miriel Duskwhisper. Each card displays the character's stats, attack, and key features. A BEGIN ENCOUNTER button at the bottom activates only once both an encounter type and a character have been selected.

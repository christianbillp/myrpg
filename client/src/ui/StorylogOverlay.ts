...
export class StorylogOverlay {
  ...
  constructor(
    ...
    this.generateBtn.className = 'gui-btn-overlay';
    ...
    if (DevMode.enabled) {
      ...
      rewriteBtn.textContent = '[DEV] REWRITE ALL';
      rewriteBtn.className = 'gui-btn-ghost';
      rewriteBtn.style.cssText += 'position:absolute;bottom:14px;right:16px;font-size:10px;padding:4px 10px;';
      ...
      this.panelEl.appendChild(rewriteBtn);
    }
    ...
    for (const record of ordered) {
      ...
      parts.push(`
        <div style="margin-bottom:32px;padding-bottom:32px;border-bottom:1px solid #1a2233;">
          <div style="font-size:9px;color:#445566;letter-spacing:1px;margin-bottom:12px;">${date.toUpperCase()}  ·  ${record.encounterTitle.toUpperCase()}</div>
          ${entry

...
export class CharSelectScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(W / 2, 48, 'CHOOSE YOUR CHARACTER', {
  ...
  private buildCard(def: PlayerDef, cx: number, cy: number, features: string[]): void {
    const cardW = 300;
    const cardH = 520;

    const bg = this.add.rectangle(cx, cy, cardW, cardH, 0x111122)
    ...
    bg.on('pointerdown', () => this.scene.start('GameScene', { playerDef: def }));
    ...
    // Name
    this.add.text(cx, top + 104, def.name, {
      fontSize: '16px', color: '#ffffff', fontFamily: 'monospace', resolution: DPR,
    }).setOrigin(0.5, 0);
    ...
    statsLines.forEach((line, i) => {
      this.add.text(cx, top + 170 + i * 22, line, {
        fontSize: '12px', color: '#aabbcc', fontFamily: 'monospace', resolution: DPR,
      }).setOrigin(0.5, 0);
    });
    ...
    features.forEach((line, i) => {
      this.add.text(cx, top + 244 + i * 26, line, {
    ...
    this.add.text(cx, top + cardH - 36, 'CLICK TO PLAY', {

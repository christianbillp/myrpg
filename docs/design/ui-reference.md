...
| **Action Buttons** | Context-sensitive combat buttons shown above CHARACTER/SEARCH (see below). Replaced by a **Spell Targeting Prompt** ("Select target for: SPELL_NAME") while spell-targeting mode is active. For attack-roll spells the prompt waits for a creature click; for AOE spells the affected tiles preview as an orange chebyshev disc that follows the cursor (or stays anchored on the player for self-range spells like Burning Hands), and clicking any tile fires the spell on that area. ESC cancels in either mode. (All targeting modes — creature / AOE / multi-projectile / summon-direct / deploy-gear / feature teleport — are polymorphic `TargetingMode` classes in `client/src/systems/TargetingModes.ts`; the Game Scene holds one active mode and delegates clicks/previews to it.) |
...
| **Description**    | Italic flavour text — how the creature looks. `NPCDef.description` when authored, falling back to the `monsterClass`'s `MonsterDef.description`; the whole block (and its separators) is hidden when neither exists. Replaces the old Combat Stats (AC/Speed) and Ability Scores blocks — the panel is now a *picture* of the creature, not a stat sheet. |
...
Defined in `client/src/ui/QuestLogOverlay.ts` (extends `BaseOverlay`); opened from the **OBJECTIVE line** in the Player Panel (which is clickable and underlines on hover). Read-only quest journal: **active quests** first, each as a card with title, description, and a **step checklist** that shows only goals the player has been made aware of — completed steps (`✓`) and the current one (`▸`); not-yet-reached steps and undiscovered optional side-goals stay hidden (the `GameScene.openQuestLog` view-model filters to `done || current`), and a found bonus appears as a completed step tagged **OPTIONAL**; then **completed / failed** quests dimmed; then a **Journal** section of prior-chapter summaries in adventure mode. The host (`GameScene.openQuestLog`) builds the view-model from `GameState.quests` resolved against the quest defs (authored from the `quests` registry, runtime from `GameState.runtimeQuestDefs`). The OBJECTIVE line itself shows the active quest's current step (driven engine-side by the `QuestSystem`). Quests advance through the engine / AIGM, never from this overlay.
...
2. **Configuration** — panel-wide display settings as cards. Currently two: **Compact View** (`myrpg_panel_compact_view`) — when enabled, the Player Panel renders Action Buttons as small **icon-only squares** (the action's glyph, full label on hover) laid out as a wrapping row instead of full-width labelled buttons; the throw picker stays full-width so item names remain readable. **Combat Speed** (`combatSpeed`, `client/src/animationSpeed.ts`) — a 1×/1.5×/2×/3× pill row scaling every combat-timeline duration (move tweens, speech reading dwells, turn-boundary pauses, condition-label dwells) via `scaleDuration`; cinematic story beats (fades, supertitles, focused announcements) keep their authored pacing. Takes effect immediately (read lazily per beat).
...
The NPC, Adventure, and Encounter creators share their scaffold — outer REGULAR / GENERATIVE AI tabs, chrome-bucket lifecycle, form input builders, and the AI propose/accept/reject panel — via `client/src/scenes/BaseCreatorScene.ts`; the Map and Token/Character creators are structurally different and stand alone.
...
| **BIG MAP — REGIONS section** (below STRUCTURES) | An add-and-configure list ([RegionList.ts](client/src/ui/edit/RegionList.ts)) for **multi-region big maps** (US-126). **+ ADD REGION** appends a row (2-5, defaults follow the grassland → forest → cave → dungeon → town journey); each row has a **biome cycle button** (GRASSLAND / FOREST / TOWN / CAVE / DUNGEON), a **light cycle** (AUTO ☀ / BRIGHT / DIM / DARK — AUTO leaves open biomes on the encounter light and makes cave/dungeon dark), and **✕** remove. A **size cycle button** in the header steps 48×24 → 60×28 → 72×32 → 96×40. With 2+ regions listed, GENERATE MAP composes via the multi-region composer (bands in travel order, ecotone blends, carved cave mouths, one named zone per region) and the TERRAIN / FEATURES / STRUCTURES controls above are ignored — the list says so inline. |

...
- A **RECENT RULINGS** block listing the last 10 adjudicated improvised actions (`"description" — skill, band (DC n) — succeeded/failed`), recorded by `ImprovisedActionSystem` and persisted with the world save. Drives the consistency step of the [improvised actions rule](#improvised-actions-rule): the same task keeps the same difficulty band.
...
When the player commits to a creative action no button or dedicated tool covers — kicking a brazier onto an enemy, wedging a door shut, swinging from a beam — the AIGM resolves it as a first-class action with a die and real stakes, never as pure narration and never as an automatic success or refusal. Full spec: [systems/improvised-actions.md](./systems/improvised-actions.md).

1. **Gate** — impossible-in-fiction attempts (the [prohibited list](#prohibited-actions)) get one refusal with a realistic alternative; questions stay questions (advice, no roll).
2. **Classify** — informational attempts (looking, listening, recalling, reading a person, searching) and social Influence attempts use `request_ability_check` and cost nothing. Attempts to physically change the world route through `resolve_improvised_action`. When the attempt imposes an effect *on* a creature (blinding, shoving, tripping), the creature gets to resist: player execution first, then on success `request_npc_saving_throw` (same band) before the effect is applied.
3. **Band the difficulty** — the model never invents a DC number; it passes `very_easy` … `nearly_impossible` and the engine maps the band to the SRD DC. `medium` is the default for a risky-but-reasonable stunt.
4. **Resolve** — call the tool and read the result before narrating any outcome. The engine spends the Action in combat (before the roll — a failed stunt is still a spent turn); a `Not performed` result means the Action was unavailable and the AIGM refuses in-fiction.
5. **Consequence on both outcomes** — success is enacted with state tools per the [tool-first rule](#tool-first-rule); failure still moves the fiction (noise, attention, a worse position, a soured attitude), also via tools. A flat "nothing happens" failure is acceptable only out of combat for zero-stakes attempts.
6. **Consistency** — CURRENT STATE carries a **RECENT RULINGS** block (the last 10 adjudications: description, skill, band, DC, outcome — recorded by the engine, persisted with the save). The same task keeps its band across attempts; a retry after failure needs changed circumstances or escalates one band.
...
| Activity | Cost |
|----------|------|
| `attack`, `throw_item`, `cast_spell` (action-time spell), `dash`, `dodge`, `disengage`, study, influence, utilize, `resolve_improvised_action` (in combat) | Action |
...
| `server/src/engine/ImprovisedActionSystem.ts` | First-class improvised-action resolution — difficulty band → DC mapping, Action spend in combat, roll via `rollAbilityCheck`, ruling record (`GameState.improvisedRulings`), uniform Event Log line (see [systems/improvised-actions.md](./systems/improvised-actions.md)) |

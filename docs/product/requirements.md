...
- **US-121** As a player, I want any creative free-text action I type — kicking a brazier onto an enemy, wedging a door shut, swinging from a beam — resolved as a first-class action with a fair ability check, an engine-owned DC (the GM picks a difficulty band, never a number), the proper Action cost in combat, and a real tool-enacted consequence on success *and* failure, so that creativity is a mechanic, not a courtesy. Rulings are recorded and surfaced back to the GM (the same task keeps the same difficulty across attempts, surviving save/load), and an effect imposed on a creature gives that creature an SRD-fair saving throw using its stat-block modifiers rather than landing by fiat. Spec: [design/systems/improvised-actions.md](../design/systems/improvised-actions.md).
- **US-125** As a player, I want the SRD's CR ≤ 1 humanoids and goblinoids in the bestiary with their real mechanics — goblin warriors whose blades bite harder (+1d4) when they strike with Advantage and who vanish between attacks, a goblin boss who shoves his own minions into my sword (Redirect Attack), a bugbear that grapples me on a hit (no save, escape DC 12 — a real **ESCAPE action**, Athletics or Acrobatics, with the grip ending if the grappler drops) and then hammers its captive at Advantage, a priest acolyte who spends its Divine Aid healing or blessing its side (+1d4 to their attacks and saves, real concentration), and a pirate whose Enthralling Panache can charm me so the engine itself refuses to let me strike them until their next turn — plus the drop-in hobgoblin warrior, scout, tough, and spy — so that low-level humanoid opposition fights with SRD craft instead of plain weapon swings. Nine roster additions with tokens, personas, and a demo encounter. Data shapes: [design/data-model.md](../design/data-model.md#monsters).
- **US-124** As a player, I want monsters with ranged weapons to actually use them — a skeleton looses its shortbow instead of marching into sword range, a bandit captain answers from 30 feet with his pistol — with the SRD rules attached: Disadvantage beyond normal range and within 5 feet, my Cover counting against the shot (no shot at all through Total Cover), and the creature still charging into melee whenever it can actually reach me that turn. Allies with ranged options shoot when their target is out of reach too.
- **US-123** As a player, I want a wider SRD bestiary fighting me with its real stat-block mechanics — the Zombie's Undead Fortitude (a CON save to stand back up at 1 HP unless the blow was radiant or a crit), the Ghoul's paralyzing Claw (a fair saving throw, paralysis that expires at the end of my next turn, and a monster smart enough to claw first and bite the paralyzed), the Shadow's Strength drain (real score loss that persists through save/load, kills at 0, and recovers on a Long Rest), and stat-block condition immunities that are actually enforced (no poisoning a zombie, no frightening an animated armor — by spell, shove, or GM fiat alike) — so that monsters feel like their SRD selves rather than re-skinned bags of hit points. Ships with five Sundered Reach roster additions: Zombie, Ghoul, Shadow, Animated Armor, and Bandit Captain. Data shapes: [design/data-model.md](../design/data-model.md#monsters).
